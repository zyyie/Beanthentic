...
class GIDocumentTrainer:
    ...
    def create_sample_dataset(self) -> List[Dict]:
        ...
        logger.info(f"Created sample dataset with {len(sample_documents)} documents")
    ...
    def load_dataset(self, create_sample_if_missing: bool = True) -> List[Dict]:
        ...
        logger.info(f"Loaded dataset with {len(dataset)} documents")
        return dataset
    ...
    def extract_features_from_dataset(self, dataset: List[Dict]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.info(f"Labels distribution: {np.bincount(y)} (Not Ready: {np.bincount(y)[0]}, Ready: {np.bincount(y)[1]})")
    ...
    def prepare_training_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.info(f"Training data saved to {self.processed_data_path}")
        return X, y
    
    def train_model(self, X: np.ndarray = None, y: np.ndarray = None) -> Dict:
        ...
        logger.info(f"Training completed. Accuracy: {accuracy:.3f} ± {cv_scores.std():.3f}")
    ...
    def create_real_dataset_template(self) -> str:
        ...
        logger.info(f"Dataset template created at {template_path}")
