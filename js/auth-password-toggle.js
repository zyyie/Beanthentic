 ...
 * Default: password hidden, eye-slash icon. Click to show/hide.
 */
(function () {
  const EYE_OPEN =
  ...
  function syncToggle(btn, input) {
    ...
    if (icon) {
      icon.innerHTML = hidden ? EYE_SLASH : EYE_OPEN;
    }
  ...
  window.initPasswordToggles = function (pairs) {
    (pairs || []).forEach(function (pair) {
      const btnId = pair[0];
      const inputId = pair[1];
      bindPasswordToggle(document.getElementById(btnId), document.getElementById(inputId));
    });
  };
